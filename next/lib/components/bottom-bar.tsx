...
const BottomBar = (props) => {
    ...
    const [_cities, setCities] = useQueryParam('cities', withDefault(StringParam, defaultValues.cities));
    ...
    const createPanoramaIndexButtonClickCB = (d: number) => () => (
        dispatchMain({ action: 'SET_PANORAMA_INDEX', payload: panoramaIndex + d })
    );
    ...
    const overlayButtonClickCB = useCallback(() => dispatchMain({ action: 'SET_OVERLAY', payload: true}), []);
    ...
    let nextUrl = nextId && idToUrl(config, nextId, searchParams)
    ...
    const prevUrl = prevId && idToUrl(config, prevId, searchParams)

    const ItemControls = (
        <div key="item">
            <Box sx={sxBottomBarGroupBody}>
                <Tooltip title="prev" position="top-center">
                    <IconButton disabled={!prevUrl} component={Link} href={prevUrl || ''} size="large"><NavigationArrowBack /></IconButton>
                </Tooltip>
                <Typography variant="body1" sx={{ display: 'inline', flexShrink: 1 }} noWrap>{ title }</Typography>
                <Tooltip title="mext" position="top-center">
                    <IconButton disabled={!nextUrl} component={Link} href={nextUrl || ''} size="large"><NavigationArrowForward /></IconButton>
                </Tooltip>
            </Box>
        </div>
    );
