...
const PanoramaBox = () => {
  ...
  const interpolatePoints = (
    pt1: google.maps.LatLng,
    pt2: google.maps.LatLng,
    r: number,
  ) => ({
    lat: r * pt2.lat() + (1 - r) * pt1.lat(),
    lng: r * pt2.lng() + (1 - r) * pt1.lng(),
  })
  ...
  const updatePath = (item: WalkT | null) => {
    ...
    if (!item) {
      const pnrm = overlay
        ? mapState.map.getStreetView()
        : refs.current.panorama
      if (pnrm) {
        pnrm.setVisible(false)
      }
    ...
    dispatchMain({
      ...
      payload: refs.current.panoramaPointsAndHeadings.length,
    })
  ...
  useEffect(() => {
    if (mapLoaded && !refs.current.panorama) {
      ...
      refs.current.panorama = new google.maps.StreetViewPanorama(
        bodyRef.current,
        {
          addressControl: true,
  ...
  return (
    <div>
      <div>
        <FormControlLabel
          control={<Switch checked={overlay} onChange={handleOverlayChange} />}
          label="overlay"
        />
      </div>
      <Box
        data-testid="panorama-box"
        sx={{
          width: '100%',
          height: '30vh',
      ...
      >
        ...
        <Typography variant="body2" style={{ flexGrow: 1 }}>
          <span>{panoramaIndex + 1} </span> / <span>{panoramaCount} </span>
        </Typography>
        <IconButton
          data-testid="forward-1-button"
          onClick={panoramaIndexButtonClickCBs['+1']}
          size="large"
        >
