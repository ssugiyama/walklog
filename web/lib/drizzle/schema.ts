import { pgTable, check, varchar, index, serial, doublePrecision, text, date, timestamp, boolean, customType } from 'drizzle-orm/pg-core'
...
const linestring = customType<{
  ...
  fromDriver(data: string): Position[] {
    ...
    const geoJson = Geometry.parse(Buffer.from(data, 'hex')).toGeoJSON() as LineString
...
const multipolygon = customType<{
  ...
  fromDriver(data: string): Position[][][] {
    // Convert WKB (Well-Known Binary) from DB back to GeoJSON coordinates for application
    const geoJson = Geometry.parse(Buffer.from(data, 'hex')).toGeoJSON() as MultiPolygon
    return geoJson.coordinates
  },
})
...
export const areas = pgTable('areas', {
  ...
  index('areas_the_geom_idx').using('gist', table.theGeom.asc().nullsLast().op('gist_geometry_ops_2d')),
])
...
export const walks = pgTable('walks', {
  ...
  index('walks_date').using('btree', table.date.asc().nullsLast().op('date_ops')),
  index('walks_draft').using('btree', table.draft.asc().nullsLast().op('bool_ops')),
  index('walks_path').using('gist', table.path.asc().nullsLast().op('gist_geometry_ops_2d')),
  check('enforce_dims_path', sql`st_ndims(path) = 2`),
  check('enforce_geotype_path', sql`(geometrytype(path) = 'LINESTRING'::text) OR (path IS NULL)`),
