...
describe('ItemFetcher', () => {
  ...
  it('force-refreshes the id token and re-fetches the item once when the token has expired, instead of getting stuck', async () => {
  ...
  it('stops retrying after one failed refresh attempt, instead of looping the item fetch forever', async () => {
    ;(getItemAction as Mock)
      .mockResolvedValueOnce(expiredResult(1))
    ...
    expect(getItemAction).toHaveBeenCalledTimes(2)
