...
const ToolBox = (props) => {
  ...
  const toggleRecordCB = useCallback(() => {
    if (!autoGeolocation && navigator.geolocation) {
      ...
      dispatchMain({
        type: 'OPEN_SNACKBAR',
        payload: 'stop following your location',
      })
    }
    dispatchMain({
      type: 'SET_AUTO_GEOLOCATION',
      payload: !autoGeolocation,
    })
  ...
  const locationChangeCB = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => setLocation(e.target.value),
    [],
  )
  const submitLocationCB = useCallback(
  ...
  return (
    <Drawer variant="persistent" anchor="left" {...props}>
      ...
      <List dense subheader={<ListSubheader>path</ListSubheader>}>
        <ListItem>
          <ListItemButton
      ...
      <Divider />
      <List dense subheader={<ListSubheader>move</ListSubheader>}>
        <ListItem>
          <ListItemIcon>
            <SearchIcon />
          </ListItemIcon>
      ...
      <Divider />
      <List dense subheader={<ListSubheader>version</ListSubheader>}>
        <ListItem>
          <ListItemIcon>
            <Numbers />
          </ListItemIcon>
