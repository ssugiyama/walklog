...
export async function handleRoute(itemId, query, isPathSelected, rows, queryChanged, next, searchFunc) {
    ...
    return next(search({ props: searchForm, func: searchFunc }));
}
...
const formWatchMiddleware = store => next => action => {
    ...
    if (!keys.every(key => payload[key] === undefined || payload[key] === state.searchForm[key])) {
        ...
        } else if (currentFilter !== 'frechet' && currentFilter !== 'hausdorff' && state.searchForm.order === 'nearest_first') {
